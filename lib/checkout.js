// Lógica de precios del carrito, compartida entre api/checkout.js (que sí crea
// el pedido) y api/cart-preview.js (que solo muestra el total en vivo, sin
// tocar la base de datos). Nunca confía en precios/cantidades que mande el
// cliente: siempre relee productos, tarifa de envío y cupón desde la BD.
...
async function computeTotals({ items, shipping_rate_id, coupon_code }) {
  ...
  const products = await sql`
    SELECT id, name, price_cop, stock, status FROM products WHERE id = ANY(${productIds})
  `;
